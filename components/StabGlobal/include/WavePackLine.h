...
protected:
	...
	struct t_WPLineRec{
		friend inline std::wostream& operator<<(std::wostream& str, t_WPLineRec rec){
		...
		t_WPLineRec(const t_FldRec& rMF, const t_WCharsGlob& rWC, const t_Index& rInd):
	...
	void _add_node(std::vector<t_WPLineRec>& add_to, 
				   const t_FldRec& fld_rec, 
				   const t_WCharsGlob& wave_chars, 
		           const t_Index& nearest_index){
						add_to.push_back(
							t_WPLineRec(fld_rec, wave_chars, nearest_index));
	};
